...
<div class="min-h-screen flex items-center justify-center p-4">
  <div class="w-full max-w-md space-y-6">
    <div class="text-center space-y-2">
      ...
      <p class="text-muted-foreground">Gérez votre session sur plusieurs appareils</p>
    </div>
    ...
    <Card>
      ...
      <CardContent class="space-y-4">
        {#if form?.code}
          <div class="p-6 bg-primary/10 border border-primary/20 rounded-lg text-center space-y-2 animate-in fade-in zoom-in duration-300">
            <div class="text-sm font-medium uppercase tracking-wider text-muted-foreground">Code de transfert</div>
            <div class="text-4xl font-mono font-bold tracking-[0.2em] py-2">{form.code}</div>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Entrez un code généré sur un autre appareil pour récupérer vos événements.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form method="POST" action="?/useCode" use:enhance class="space-y-4">
          ...
          <Button type="submit" class="w-full" variant="outline" disabled={inputCode.length < 6}>
    ...
    <div class="pt-8 text-center">
      <details class="text-xs text-muted-foreground">
        <summary class="cursor-pointer hover:underline">Informations avancées</summary>
